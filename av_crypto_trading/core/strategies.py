...
def strategy_naive(
    ...
    while True:
        df = binance_api.get_minute_data(
            currency=currency.value, interval="1m", lookback="40", db=db
        )
        if not open_position:
            ...
            if cuml_ret[-1] < -0.002:
                ...
                open_position = True
            utils.print_iter(value=cuml_ret[-1], lower="< -0.002",every=1)
        else:
            ...
            if len(since_buy) > 0:
                ...
                if since_buy_ret[-1] > 0.0015 or since_buy_ret[-1] < -0.0015:
                    ...
                    return database_api.get_profit_report(
                        buy_order, sell_order, currency.value, quantity
                    )
...
def strategy_trailing_stop_loss(
    ...
    while True:
        if not open_position:
            df = binance_api.get_minute_data(
                currency=currency.value, interval="1m", lookback=str(look_back), db=db
            )
...
def strategy_Stoch_RSI_MACD(
    ...
    while True:
        if not open_position:
            df = binance_api.get_minute_data(
                currency=currency.value, interval="1m", lookback="100", db=db
            )
        ...
        if open_position:
            time.sleep(0.5)
            df = binance_api.get_minute_data(
                currency=currency.value, interval="1m", lookback="2", db=db
            )
...
def strategy_top_symbol_high_risk(
    ...
    while True:
        if not open_position:
            ...
            df = binance_api.get_minute_data(
                currency=currency.value, interval="1m", lookback="120", db=db
            )
            ...
            if ((df.close.pct_change() + 1).cumprod()).iloc[-1] > 1:
                ...
                buy_order = binance_api.make_order(
                    currency=currency.value, decision=contants.Decision.BUY, quantity=quantity
                )
            ...
            sub_df = database_api.get_crypto_from_starting_time(
                currency.value, buy_order["transaction_time"]
            )
            utils.print_iter(
...
def strategy_SMA_crossover(
    ...
    while True:
        sub_df = database_api.get_crypto_from_starting_time(currency.value, starting_time)
        if not sub_df.empty:
...
def strategy_FastSMA_SlowSMA_long_interval(
    ...
    while True:
        ...
        df = binance_api.get_minute_data(
            currency=currency.value, interval="1h", lookback="45000", db=db
        )
