...
def update_position(
    currency: contants.Currencies, position: int, quantity: float
) -> None:
...
def change_position(
    currency: contants.Currencies, order: dict, decision: contants.Decision
):
...
def get_profit_report(
    buy_order: dict, sell_order: dict, currency: contants.Currencies, quantity: float
...
def get_currency_price_from_lookback(
    currency: contants.Currencies, lookback: int
) -> pd.DataFrame:
...
def get_crypto_from_starting_time(
    currency: contants.Currencies, starting_time: Timestamp
) -> pd.DataFrame:
    df = pd.read_sql(
        f"""
            SELECT * FROM {contants.Tables.cryptocurrencies}
